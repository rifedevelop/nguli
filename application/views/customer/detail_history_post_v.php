        ...
        <div class="row px-4 pb-5 mb-3 border-bottom">
            ...
            <div class="col-md-6 pb-3">
                <div class="d-flex flex-column text-right">
                    <h6 class="poppins-bold mt-0">IDR <?=number_format($dataPost->budget_min, 0, '', '.');?> - <?=number_format($dataPost->budget_max, 0, '', '.');?> </h6>
                    <h6>Jenis Proyek : <?= $dataPost->category_name;?></h6>
                    <h6>Luas Tanah : <?=$dataPost->area;?> m2</h6>
                    <h6><i class="fa fa-clock-o" style="font-size:1.3rem"></i> <span class="color-red mx-1">Tutup Tender</span> <?=$dataPost->selisih_tgl.' hari ';?>
                    <?php 
                        $waktu_awal = strtotime($dataPost->now);
                        $waktu_akhir = strtotime($dataPost->close_tender);
                        $diff = $waktu_akhir - $waktu_awal;
                        $jam = floor($diff / (60 * 60));
                        $menit=$diff - $jam * (60 * 60);
                        // DATEDIFF(minute,$waktu_awal,$waktu_akhir;
                        $datetime1 = date_create($dataPost->now);
                        $datetime2 = date_create($dataPost->close_tender);
                        $interval = date_diff($datetime1, $datetime2);
                        echo $interval->format('%h').' jam lagi';
                    ?>
                    </h6>
                </div>
            </div>
            ...
            <div class="col-md-12 mb-3">
                ...
                <div class="row">
                    ...
                    ?>
                        <div class="col-md-2 px-2 py-3">
                            <img class="img-fluid" src="<?=base_url().'uploads/customer/post/'.$val->img;?>" style="width:150px;height:150px;object-fit:cover;" />
                        </div>
                    <?php endforeach; ?>
                </div>
